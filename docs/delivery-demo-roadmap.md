# 数字人直播交付 Demo — 历史任务清单（已被生产标准架构取代）

> ⚠️ 本文为早期（观众端不做、流程可控优先）的历史路线，已被**生产标准架构**取代：
> 数智人 rtmp 流 → 专业 OBS 拉流转推 → TRTC 直播间 → 真实观众；评论真走 IM + 撤回/禁言；多管理员协作。
> 当前权威文档请见 [README](../README.md)、[progress.md](../progress.md)、[OBS 拉流转推管线](obs-pull-push-pipeline.md)、[IM 评论链路](im-comment-moderation.md)。
> 以下内容仅作历史参考。
